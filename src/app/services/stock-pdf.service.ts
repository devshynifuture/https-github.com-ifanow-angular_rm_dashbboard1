...
export class StockPdfService {
  ...
  generatePdfWithoutHtml(title, header, tableData) {
    ...
    console.log(this.imageUrl, "this.getBase64ImageFromURL(this.getOrgData.reportLogoUrl)");
    let headers = header;
    let footer = tableData.filter((element, index) => index == tableData.length - 1);
    const trTd = [tableData.filter((element, index) => index != tableData.length - 1)];
    let td = [];
    // const header = headers;
    const data = trTd;
    // for (let cells in rows) {
    ...
    const documentDefinition = {
      ...
      content: [
        ...
        // { lineHeight: 2, text: 'Investor: ' + '', style: 'clientStyle' },
        {
          layout: {
            fillColor: function (rowIndex, node, columnIndex) {
              return (rowIndex == 0) ? '#f6f7f7' : null;
