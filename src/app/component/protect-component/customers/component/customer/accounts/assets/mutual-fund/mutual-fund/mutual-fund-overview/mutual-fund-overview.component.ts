  ...
  getLatestNavDate(data) {
    ...
    if (data.schemeWise) {
      ...
      if (sortedData.length > 0) {
        const maxDateOne = new Date(sortedData[0].navDate);
  ...
  getMutualFundResponse(data) {
    ...
    if (data) {
      ...
      if (!isNaN(this.mfData.total_current_value) && !isNaN(this.mfData.total_amount_invested) && !isNaN(this.mfData.total_unrealized_gain) && !isNaN(this.mfData.total_absolute_return)) {
        ...
        this.mfData.total_unrealized_gain_Dupl = this.mfData.total_unrealized_gain;
  ...
  getCashFlowStatus() {
    ...
    if (this.totalValue) {
      this.datasource1.data = [
        ...
        {
          data: 'e. Dividend Payout',
          amts: (this.cashFlowObj.cashFlowDividendPayout) ? this.cashFlowObj.cashFlowDividendPayout : 0
        },
        {
        ...
        {
          data: 'g. Market Value',
          amts: (this.cashFlowObj.cashFlowMarketValue) ? this.cashFlowObj.cashFlowMarketValue : 0
        },
        { data: 'h. Net Gain (g-f)', amts: (this.cashFlowObj.cashFlowNetGain) ? this.cashFlowObj.cashFlowNetGain : 0 },
        {
          data: 'i. Lifetime XIRR (All Transactions)',
          amts: (this.cashFlowObj.cashFlowLifetimeXirr) ? this.cashFlowObj.cashFlowLifetimeXirr : 0
        },
  ...
  openMutualFund(flag, data) {
    ...
    switch (flag) {
      ...
      case 'addMutualFund':
        ...
        break;
      case 'import-cas-file':
        component = MfImportCasFileComponent;
        break;
      default:
