...
export class BondsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.bonds.get('tenureInYear').valueChanges
      .subscribe(year => {
        const month = this.bonds.get('tenureInMonth');
        if (year.length == 0) {
          month.setValidators([Validators.required, Validators.min(1), Validators.max(12)])
        }
        else {
          month.clearValidators()
          month.setValidators([Validators.min(1), Validators.max(12)])
        }
        month.updateValueAndValidity();
      })
    this.bonds.get('tenureInMonth').valueChanges
      .subscribe(month => {
        const year = this.bonds.get('tenureInYear');
        if (month.length == 0) {
          year.setValidators([Validators.required, Validators.min(1), Validators.max(120)])
        }
        else {
          year.clearValidators()
          year.setValidators([Validators.min(1), Validators.max(120)])
        }
  ...
  saveBonds() {
    ...
    if (this.bonds.invalid) {
      ...
      let obj = {
        ...
        tenureInYear: parseInt(this.bonds.value.tenureInYear),
        tenureInMonth: parseInt(this.bonds.value.tenureInMonth),
        // tenure: Number(this.bonds.controls.tenure.value),
