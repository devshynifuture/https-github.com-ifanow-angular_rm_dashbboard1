...
export class MotorInsuranceComponent implements OnInit {
	...
	getdataForm(data) {
		...
		this.motorInsuranceForm = this.fb.group({
			...
			policyName: [(this.dataForEdit ? this.dataForEdit.planName : null), [Validators.required]],
			...
			declaredValue: [(this.dataForEdit ? this.dataForEdit.sumInsuredIdv : null)],
	...
	saveMotorInsurance() {
		...
		if (this.motorInsuranceForm.invalid) {
			...
			const obj = {
				...
				planName: this.motorInsuranceForm.get('policyName').value,
				...
				sumInsuredIdv: parseInt(this.motorInsuranceForm.get('declaredValue').value),
