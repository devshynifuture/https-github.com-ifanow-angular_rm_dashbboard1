...
export class AddSuggestPolicyComponent implements OnInit {
  ...
  getdataForm(data) {
    ...
    this.suggestPolicyForm = this.fb.group({
      ...
      implementationDate: [(this.dataForEdit ? new Date(this.dataForEdit.implementationDate)  : null), [Validators.required]],
    })
    if (this.dataForEdit) {
  ...
  saveSuggestPolicy() {
    if (this.suggestPolicyForm.invalid) {
      ...
      const obj = {
        ...
        'implementationDate': this.suggestPolicyForm.get('implementationDate').value,
        'realOrFictitious': 2,
