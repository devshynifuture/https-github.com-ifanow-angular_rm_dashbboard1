...
export class LifeInsuranceComponent implements OnInit {
  ...
  callApiData(data) {
    ...
    const refreshData = this.isRefreshRequired;
  ...
  openDetailedView(data) {
    if (data && data.adviceDetails) {
      if (data.adviceDetails.gen_insurance_advice_id) {
        const id = data ? (data.adviceDetails ? (data.adviceDetails.gen_insurance_advice_id) : this.adviceName) : this.adviceName;
  ...
  checkAndPushRecommendationData(array, id) {
    const singleData = this.storedData.filter(d => d.id == this.inputData.id);
  ...
  openAddEditAdvice(value, data, flag) {
    ...
    const fragmentData = {
      ...
      childData: {
        ...
        displayList: this.displayList,
        insuranceSubTypeId: this.object.insuranceSubTypeId,
        insuranceTypeId: 2,
  ...
  deleteModalRecommendations(value, data) {
    const deletedAdviceId = data ? (data.adviceDetails ? data.adviceDetails.id : null) : null;
    if (data) {
      data.inputData = this.inputData;
    } else {
      data = { inputData: this.inputData };
  ...
  generatePdf() {
    ...
    const para = document.getElementById('insuranceTemplate');
  ...
  getDetailsInsurance() {
    ...
    forkJoin(detailofInsurance, sendPolicy, sendRecommendation, needAnalysis).subscribe((result: any) => {
  ...
  getForkJoinResponse(result) {
    ...
    this.panelOpenState = false;
    const suggestedData = result[1];
    this.isLoadingPlan = false;
    if (suggestedData) {
      if (this.inputData.insuranceType != 1) {
        const current = suggestedData.current.length > 0 ? suggestedData.current[0] : [];
        const suggested = suggestedData.suggested.length > 0 ? suggestedData.suggested : [];
        const mergeArray = [...suggested];
        mergeArray.forEach(element => {
          element.insurance = element.insuranceDetails;
          element.expanded = false;
        });
        this.dataSource3 = this.getFilterDataNeedAnalysis(mergeArray);
      } else {
        ...
        this.dataSource3 = suggestedData;
      }
      let countSuggest = 0;
      if (this.dataSource3.length > 0) {
        let suggestionId;
        this.dataSource3.forEach(ele => {
        ...
        const deletedArray = this.dataSource3.filter(d => d.insurance.id == suggestionId);
        let RemovedArray = this.dataSource3.filter(d => d.insurance.id != suggestionId);
        RemovedArray.unshift(deletedArray);
        RemovedArray = RemovedArray.flat();
        this.dataSource3 = RemovedArray;
      }
    } else {
      this.dataSource3 = [];
    }
  ...
  setAdviceAmountToAllIns() {
    const singleData = this.allInsuranceData.filter(d => d.id == this.inputData.id);
    ...
    if (this.insuranceDetails && this.insuranceDetails.needAnalysis.needTypeId == 1) {
      ...
      const actualAmount = this.insuranceDetails.actual ? this.insuranceDetails.actual : 0;
      const adviceAmount = this.insuranceDetails.advice ? this.insuranceDetails.advice : 0;
