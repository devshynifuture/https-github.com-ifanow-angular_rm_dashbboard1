...
import { AddSettingSchemeBasketComponent } from '../../../setting-entry/add-setting-scheme-basket/add-setting-scheme-basket.component';
...
export class SchemeBasketComponent implements OnInit {
  ...
  openAddSchemeDetails(value, data) {
    let popupHeaderText = !!data ? 'Edit Fixed deposit' : 'Add Fixed deposit';
    const fragmentData = {
      flag: value,
      data,
      id: 1,
      state: 'open45',
      componentName: AddSettingSchemeBasketComponent,
      popupHeaderText: popupHeaderText,
    };
    const rightSideDataSub = this.subInjectService.changeNewRightSliderState(fragmentData).subscribe(
      sideBarData => {
