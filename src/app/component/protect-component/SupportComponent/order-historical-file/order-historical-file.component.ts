...
export class OrderHistoricalFileComponent implements OnInit {
  ...
  resetDateAsOnDateAndFormCheckbox() {
    if (this.orderHistoryFileForm.get('selectRta').value !== this.camsRtId) {
      ...
      if (this.orderHistoryFileForm.get('selectRta').value === this.franklinRtId) {
        this.camsAsOnDatePastMaxDate = new Date("1 January, 1990");
      } else {
  ...
  ngOnDestroy(): void {
    if (this.camsValueChangeSubscription) {
      this.camsValueChangeSubscription.unsubscribe();
    }
    if (this.karvyValueChangeSubscription) {
      this.karvyValueChangeSubscription.unsubscribe();
    }
    if (this.franklinValueChangeSubscription) {
  ...
  calculateDateAndObject() {
    ...
    if (this.orderHistoryFileForm.value.hasOwnProperty('fromDate') && this.orderHistoryFileForm.value.hasOwnProperty('toDate')) {
        ...
        // monthly
          ...
          if (fromDateString !== 1) {
              ...
              for (let index = 1; index <= yearDiffr; index++) {
                ...
                for (let index1 = 1; index1 <= 12; index1++) {
                  if (index1 === 1) {
                    ...
                    fromDateIter = this.addYearMonthOrDayToDate(toDateIter, 1, 'day');
                    let toDateIterTemp = this.addYearMonthOrDayToDate(fromDateIter, 1, 'month')
                    if (toDateIterTemp.getTime() >= toDateValueObj.getTime()) {
                      toDateIter = toDateValueObj;
                    } else {
                      toDateIter = this.addYearMonthOrDayToDate(fromDateIter, 1, 'month');
                    }
            ...
            if (yearDiffr === 0) {
              ...
              for (let index2 = 1; index2 <= yearDiffr; index2++) {
                // is 
                for (let index1 = 1; index1 <= 12; index1++) {
                  if (index1 === 1) {
                    fromDateIter = fromDateValueObj;
      ...
      let asOnDate = this.orderHistoryFileForm.get('asOnDate').value._d;
      const fileToOrder = this.orderHistoryFileForm.get(`selectFilesToOrder.${whichRta}`).value;
      let fileTypeId;
      let rtId;
      if (fileToOrder) {
            ...
            if (fileTypeId === 6 || fileTypeId === 12) {
              if (!(requestObj.some(item => item.fileTypeId === fileTypeId))) {
                requestObj.push({
                  ...
                  arnRiaDetailId: this.arnRiaDetails,
                  fromDate: null,
                  toDate: asOnDate.getFullYear() + "-" + this.util.addZeroBeforeNumber((asOnDate.getMonth() + 1), 2) + "-" + this.util.addZeroBeforeNumber(asOnDate.getDate(), 2),
                  fileTypeId: fileTypeId,
                  orderingFrequency: 3
                });
              }
